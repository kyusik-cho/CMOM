...
from cmom.dataset.Viper import ViperDataSet
from cmom.dataset.SynthiaSeq import SynthiaSeqDataSet
from cmom.dataset.CityscapesSeq import CityscapesSeqDataSet
from cmom.domain_adaptation.config import cfg, cfg_from_file
from cmom.domain_adaptation.train_video_UDA import train_domain_adaptation
...
def main():
    ...
    if cfg.TRAIN.MODEL == 'ACCEL_DeepLabv2':
        ...
        student_path = cfg.TRAIN.RESTORE_FROM     
        assert osp.exists(student_path), f'Missing init model {student_path}'
        saved_state_dict = torch.load(student_path)
        if 'DeepLab_resnet_pretrained_imagenet' in student_path:
