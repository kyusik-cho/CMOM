...
class ResNetMulti(nn.Module):
    ...
    def forward(self, cf, kf, flow, device, need_prev_pred = False):
        cf = self.conv1(cf)
        cf = self.bn1(cf)
        cf = self.relu(cf)
        cf = self.maxpool(cf)
        ...
        if self.multi_level:
            ...
            cf_aux = None
        cf_feature = self.layer4(cf)
        cf = self.layer6(cf_feature)
        with torch.no_grad(): 
            kf = self.conv1(kf) 
            kf = self.bn1(kf)
            kf = self.relu(kf)
            ...
            if self.multi_level:
                ...
                kf_aux = None
            kf_feature = self.layer4(kf)
            kf = self.layer6(kf_feature)
        ...
        # Option for generating pseudo label
        if need_prev_pred:
            with torch.no_grad():
                kf_aux2 = kf_aux
                kf2 = kf
                prev_pred_aux = self.sf_layer(torch.cat((kf_aux, kf_aux2), dim=1))
                prev_pred = self.sf_layer(torch.cat((kf, kf2), dim=1))

        if need_prev_pred:
            return pred_aux, pred, cf_aux, cf, cf_feature, kf_aux, kf, kf_feature, prev_pred_aux, prev_pred
        return pred_aux, pred, cf_aux, cf, cf_feature, kf_aux, kf, kf_feature
