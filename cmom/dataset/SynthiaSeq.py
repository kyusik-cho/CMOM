...
class SynthiaSeqDataSet(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        label_file_S_kf = str(label_file).replace(str(frame).zfill(6) + '.png', str(frame - 1).zfill(6) + '.png')
        label_kf_S = self.get_labels_synthia_seq(label_file_S_kf)
        label_kf_S = label_kf_S[:-120, :]
